...
{
    /// <summary>
    /// <para type="synopsis">Initializes one or more <see cref="PrtgClient"/> instances for connecting to PRTG servers. This cmdlet must be called at least once before attempting to use any other cmdlets.</para>
    ///
    /// <para type="description">The Connect-PrtgServer cmdlet establishes connections to one or more PRTG Servers. As PRTG uses a stateless REST API,
    /// Connect-PrtgServer initializes <see cref="PrtgClient"/> instances which store the settings that will be used when generating each request.</para>
    ///
    /// <para type="description">When multiple servers are specified, all subsequent PrtgAPI cmdlets will execute against all connected servers in parallel by default.
    /// Each object returned from cmdlets will be automatically tagged with its source server. When objects are piped between cmdlets, operations will automatically
    /// target the server that each object originated from. You can also explicitly specify which server(s) to target using the -Client parameter available on all cmdlets.</para>
    ///
    ...
    /// </example>
    /// <example>
    ///     <code>C:\> Connect-PrtgServer prtg1.example.com,prtg2.example.com (New-Credential prtgadmin prtgadmin)</code>
    ///     <para>Connect to multiple PRTG Servers with the same credentials. All subsequent PrtgAPI cmdlets will execute against both servers in parallel.</para>
    ///     <para/>
    /// </example>
    /// <example>
    ///     <code>C:\> Get-Sensor</code>
    ///     <para>Retrieve sensors from all connected PRTG servers in parallel. Each returned sensor will be tagged with its source server.</para>
    ///     <para/>
    /// </example>
    /// <example>
    ///     <code>C:\> Get-Sensor -Client "prtg1.example.com"</code>
    ///     <para>Retrieve sensors from only the specified PRTG server.</para>
    ///     <para/>
    /// </example>
    /// <example>
    ///     <code>C:\> Get-Sensor | Set-ObjectProperty -Property Active -Value $false</code>
    ///     <para>When objects are piped from one cmdlet to another, operations automatically target the server that each object came from.</para>
    ///     <para/>
    /// </example>
    ///
    ...
    {
        /// <summary>
        /// <para type="description">Specifies one or more PRTG Servers to connect to. If the server does not use HTTPS, http:// must be specified.</para>
        /// </summary>
        [Parameter(Mandatory = true, Position = 0, HelpMessage = "The address(es) of the PRTG Server(s) to connect to. If the server does not use HTTPS, http:// must be specified.")]
        public string[] Server { get; set; }

        /// <summary>
        /// <para type="description">Specifies the username and password to authenticate with. If <see cref="PassHash"/> is specified, the password will be treated as a PassHash instead. If multiple servers are specified and different credentials are needed, pipe hashtables with Server and Credential properties.</para>
        /// </summary>
        ...
        private List<PrtgClient> connectedClients = new List<PrtgClient>();

        /// <summary>
        /// <para type="description">Returns the <see cref="PrtgClient"/> that was created by this cmdlet.</para>
        /// </summary>
        public object PassThruObject => connectedClients.Count == 1 ? (object)connectedClients[0] : connectedClients.ToArray();
        ...
        public void WritePassThru()
        {
            if (PassThru)
            {
                if (connectedClients.Count == 1)
                    WriteObject(connectedClients[0]);
                else
                    WriteObject(connectedClients, true);
            }
        }
        ...
        {
            // Check if we're already connected to any of the requested servers
            if (!Force.IsPresent)
            {
                var existingServers = Server.Where(s => PrtgSessionState.ClientManager.HasClient(s)).ToList();
                if (existingServers.Any())
                {
                    var serverList = string.Join(", ", existingServers);
                    throw new InvalidOperationException($"Already connected to server(s): {serverList}. To override please specify -Force.");
                }
            }

            // Set PSEdition once for all connections
            if (PrtgSessionState.PSEdition == null)
                PrtgSessionState.PSEdition = GetPSEdition(cmdlet);

            // Set progress preference once
            ...
            bool isFirstConnection = PrtgSessionState.Client == null;
            ...
            foreach (var server in Server)
            {
                ...
                var client = new PrtgClient(server, cred.UserName, cred.Password, authMode, IgnoreSSL);

                if (RetryCount != null)
                    client.RetryCount = RetryCount.Value;

                if (RetryDelay != null)
                    client.RetryDelay = RetryDelay.Value;
                ...
                // Add or update the client in the manager
                if (Force.IsPresent)
                    PrtgSessionState.ClientManager.AddOrUpdateClient(client, setAsDefault: isFirstConnection && connectedClients.Count == 0);
                else
                    PrtgSessionState.ClientManager.AddClient(client, setAsDefault: isFirstConnection && connectedClients.Count == 0);

                connectedClients.Add(client);

                if (client.Server != server && (client.LogLevel & PrtgAPI.LogLevel.Trace) == PrtgAPI.LogLevel.Trace)
                {
                    WriteVerbose($"{MyInvocation.MyCommand}: Server redirected HTTP to HTTPS. Changing server URL from '{server}' to '{client.Server}'");
                }
            }
