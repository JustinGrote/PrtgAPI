if(!$skipBuildModule)
{
    ipmo $PSScriptRoot\..\..\..\..\build\PrtgAPI.Build -Scope Global
}

Import-Module $PSScriptRoot\..\..\..\..\build\CI\ci.psm1 -Scope Global
...
if ((Get-Module pester).Version.Major -gt 3 -and !(Get-Command Assert-VerifiableMocks -CommandType alias -ErrorAction SilentlyContinue)) {
...
function Get-ProcessTree {
    ...
    while ($true) {
        if ($me -eq $null) {
            if ($list.Count -eq 0) {
                $me = $all | Where-Object ProcessID -EQ $pid
            } else {
                break
            }
        } else {
            $me = $all | Where-Object ProcessID -EQ $me.ParentProcessID

            if ($list | Where-Object ProcessID -EQ $me.ProcessID) {
...
function SkipBuildTest {
    ...
    if (-not (Get-ChildItem $root -Filter *.sln* -File -ErrorAction SilentlyContinue)) {
        return $true
    }
...
function WithNewProcess($command, $exe = $null) {
    ...
    & $exe -NoProfile -NonInteractive -Command "$fixPath; . '$path'; $command" | Write-Host
